...
import { ICustomersRepository, Pagination } from "@modules/customers/domain/repositories/ICustomerRepositories";
...
export default class customersRepository implements ICustomersRepository {
  ...
  async findAndCount({take, skip}: Pagination): Promise<[ICustomer[], number]> {
    const [customers, total ] = await await this.ormRepository.findAndCount({
      take,
      skip
    });
